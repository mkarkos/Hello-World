...
# needs to be run in CodeSkulptor http://www.codeskulptor.org/
import simplegui
import math
import random
...
explosion_info = ImageInfo([50, 50], [100, 100], 17, 40, True)
...
def group_group_collide(first_group, second_group):
    collisions = 0
    for item in set(first_group):
        for missile in set(second_group):
            if item.collide(missile) == True:
                collisions += 1
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated == True:
            self.image_center[0]= 64 + (self.age*100)
...
def draw(canvas):
    global time, started, lives, rock_group, missile_group, my_ship, score, explosion_group
    ...
    # detect collision between ship and rocks
    if group_collide(rock_group, my_ship) == True:
        lives -= 1
        if lives <= 0:
            started = False
            
    # detect collision between missile and rocks
...
def rock_spawner():
    ...
    if dist(rock_pos, my_ship.get_position()) < 150:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
...
frame.add_label("Directions:")
frame.add_label("Left and right arrow keys to rotate ship.")
frame.add_label("Up arrow key to move forward.")
frame.add_label("Space bar to fire missile.")                
